...
import { CheckCircle, Clock, Mail, Phone, User, AlertCircle, Package, Truck, Trash2, XCircle, ChevronDown } from 'lucide-react';
...
export default function PendingPayments() {
    ...
    const fetchOrders = async () => {
        try {
            const { data } = await api.get('/admin/orders/pending-payment');
            setOrders(data);
        } catch (error) {
    ...
    const groupOrders = () => {
        ...
        return {
            pending: orders.filter(o => o.status === 'PENDING_PAYMENT' || o.status === 'PENDING').sort(sortByMethod),
            processing: orders.filter(o => o.status === 'PROCESSING').sort(sortByMethod),
            shipped: orders.filter(o => o.status === 'SHIPPED' || o.status === 'COMPLETED').sort(sortByMethod) // Added COMPLETED here for consistency
        };
    };
    ...
    const OrderCard = ({ order, updateTracking, deleteOrder, confirmPayment, markCompleted, processing, formatDate, getStatusColor }) => {
                    ...
                    <div className="flex items-start justify-between mb-4">
                        <div>
                            <div className="flex items-center gap-2 mb-2">
                                <span className={`text-xs font-black uppercase tracking-widest ${getStatusColor(order.status)}`}>
                                    {order.status.replace('_', ' ')}
                                </span>
                                <span className="text-xs font-bold text-neutral-600">#{order.id.slice(0, 8)}</span>
                            </div>
                        ...
                        <div className="text-right">
                            ...
                            <div className="text-xs text-neutral-500 font-bold uppercase">{order.paymentMethod}</div>
                        </div>
                    </div>
